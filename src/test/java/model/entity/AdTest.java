...
public class AdTest
{
    ...
    public void setAd()
    {
        ...
        InputStream expectedData = new ReaderInputStream(new StringReader("test"));

        Ad ad = new Ad();
        ad.setID(expectedID);
        ad.setFilename(expectedName);
        ad.setFiledata(expectedData);

        assertAll(
                () -> assertEquals(ad.getID(), expectedID),
                () -> assertEquals(ad.getFilename(), expectedName),
                () -> assertEquals(ad.getFileData(), expectedData)
        );
    }
}
